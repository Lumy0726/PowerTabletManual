...
* \<ADB 연결 인증 실패 문제 해결하기\>  
  어떤 이유로 ADB 연결이 인증 실패 문제로 실패할 수 있습니다.  
  이 문제를 풀기 위해서는 아래 절차를 수행하고 다시 시도해 보세요.  
  안드로이드 기기를 PC와 연결 해제합니다.  
  윈도우에서 작업 관리자를 열고 adb.exe 프로세스를 종료합니다.  
  안드로이드의 '개발자 옵션'에서 'USB 디버깅'을 비활성화했다가 다시 활성화합니다.
